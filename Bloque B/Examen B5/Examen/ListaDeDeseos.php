...
class ListaDeDeseos
{
    ...
    public function __construct(array $palabrasProhibidas = [], array $posiblesRegalos = ["bicicleta" => 299.99, "patinete" => 499.90, "videoconsola" => 599.99])
    {
        ...
        $this->regalosDisponibles = $posiblesRegalos;
        $this->regalosDisponibles;

        for ($i = 0; $i < 2 * count($this->posiblesRegalos); $i++) {
    ...
    {
        for ($i = 0; $i < $this->posiblesRegalos; $i++) {
            if ($nombreRegalo == "carbón") {
                return 0;
            } else {
                return $this->posiblesRegalos[$nombreRegalo]; //(Devuelve el precio si encuentra el regalo).
            }
        }
    }
    ...
    {
        foreach ($this->palabrasProhibidas as $indicador) {
            $regex = preg_match("/$indicador/", $texto); // Localiza la palabra prohibida dentro del texto pasado por parametro.

            while ($regex) {
                $texto = preg_replace("/$indicador/", str_repeat("*", strlen($indicador)), $texto); // Sustituye la palaba no deseada por tantas * como longitud de la palabra.
                $regex = preg_match("/$indicador/", $texto);
                // Recorre nuevamente la frase para buscar más palabras no deceadas, si las encuentra repite el proceso
                // En caso de no encontrarlas se cancela el bucle.
            }
        }
        return $texto; //Devuelve el texto sin palabras prohibidas
    }
    ...
    {
        $clave = array_rand($this->posiblesRegalos);
        array_push($this->regalosDisponibles, $clave);
    }
    ...
    public function analisisReparto(array $repartoDeRegalos): array
    {
        ...
        for ($i = 0; $i < count($repartoDeRegalos); $i++) {
            ...
            for ($j = 0; $i < count($repartoDeRegalos[$i]); $j++) {
                $productos = $repartoDeRegalos[$i][$j];
                if (preg_match("/$productos/", $this->cartas[$i])) {
                    $repartoCumplido = true;
                    array_push($listaReparto, $repartoCumplido);
                }
            }
            array_push($listaReparto, $repartoCumplido);
        }
